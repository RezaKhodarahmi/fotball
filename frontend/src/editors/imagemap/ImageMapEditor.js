...
class ImageMapEditor extends Component {
	...
	canvasHandlers = {
      ...
      if (target.type === 'line' || target.type === 'pathArrow' || target.type === 'sineArrow' || target.type === 'polygon') {
				return (
					<Menu>
            <Menu.Item
							onClick={() => {
	...
	render() {
      ...
      onFileSave,
        ...
        <CommonButton
					...
					tooltipPlacement="bottomRight"
				>{i18n.t('action.file-save')}</CommonButton>
			</React.Fragment>
		);
		const titleContent = (
			<React.Fragment>
      ...
      <React.Fragment>
        ...
        <Modal
				...
				>
          <div>
            <Input value={this.state.editorName}
							onChange={e => {
								this.setState({editorName: e.target.value});
							}} style={{ margin: '20px 0 30px' }} placeholder={i18n.t('action.editor-name')}></Input>
            <Button onClick={onFileSave} block>{i18n.t('action.save')}</Button>
          </div>
				</Modal>
      </React.Fragment>
    );
