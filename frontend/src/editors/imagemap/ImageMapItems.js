...
class ImageMapItems extends Component {
	...
	handlers = {
		...
		onDrawingItem: item => {
			...
			} else if (item.option.type === 'arrowpath') {
				canvasRef.handler.drawingHandler.arrowpath.init();
			} else if (item.option.type === 'sinepath') {
				canvasRef.handler.drawingHandler.sinePath.init();
			} else {
	...
	renderItem = (item, centered) =>
        ...
        {item.option.src ? <img src={item.option.src} width={20} /> :
					<Icon name={item.icon.name} prefix={item.icon.prefix} style={item.icon.style} />
        }
				</span>
				{this.state.collapse ? null : <div className="rde-editor-items-item-text">{i18n.t(item.name)}</div>}
			</div>
		) : (
			<div
				key={item.name}
								...
								(collapse ? (
									...
									>
										{Object.keys(descriptors).map(key => (
											<Collapse.Panel key={key} header={i18n.t(key)} showArrow={false}>
