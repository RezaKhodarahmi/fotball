...
import { ColorPicker, CommonButton } from '../../components/common';
...
class ImageMapHeaderToolbar extends Component {
	...
	render() {
        ...
        {canvasRef && canvasRef.handler?.cropHandler.validColorable() && (
          <Flex.Item className="rde-canvas-toolbar rde-canvas-toolbar-operation">
          <ColorPicker
            onChange={(color) => canvasRef.handler?.changeColor(color)}
            value={canvasRef.handler?.getObjectColor()}
          />
				</Flex.Item>
        ) }
