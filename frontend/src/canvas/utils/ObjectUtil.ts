...
export type InteractionMode = 'selection' | 'grab' | 'polygon' | 'line' | 'arrow' | 'sineArrow' | 'link' | 'crop';
...
export interface FabricEvent<T extends any = Event> extends Omit<fabric.IEvent, 'e'> {
	...
	transform?: { corner: string; original: FabricObject; originX: string; originY: string; width: number, offsetX: number, offsetY: number };
}
