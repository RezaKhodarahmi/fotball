...
class Handler implements HandlerOptions {
    ...
    if (obj.type === 'pitch') {
      this.canvas.forEachObject((o: FabricObject, _) => {
        if (o?.type == 'pitch') { 
          this.canvas.remove(o);
        }
      })
			createdObj = this.addImage({
        ...newOption,
        lockMovementX: true,
				lockMovementY: true,
        hasBorders: false,
    ...
    } else if (obj.type === 'player' || obj.type === 'cone') {
      createdObj = this.addPlayer(newOption);
		} else if (obj.type === 'tools') {
			createdObj = this.addSvg(newOption); 
    } else if (obj.type === 'image') {
      console.log('newOption',newOption);
			createdObj = this.addImage(newOption);
		} else if (obj.type === 'group') {
			createdObj = this.addGroup(newOption);
    } else if (obj.type === 'triangle' || obj.type === 'rect' || obj.type === 'circle' || obj.type === 'textbox') {
      const shapeOption = {
        ...newOption,
        hasBorders: true,
        selectable: true,
        hasControls: true
			...
			createdObj = this.fabricObjects[obj.type].create(shapeOption);
		} else {
  ...
  public addPlayer = (obj: FabricImage) => {
    const { objectOption, name, id, top, left, fill, fillIndex } = obj;
    const options = {
      ...objectOption,
      id,
      svg: obj.svg,
  ...
  public addSvg = (obj: FabricImage) => {
    ...
    if (activeObject.type == 'circle' && activeObject.parentId) {
      return;
    }
    if (activeObject.type == 'pathArrow') {
      this.canvas.forEachObject((o: FabricObject, _) => {
        if (o.parentId == activeObject.id) {
          this.canvas.remove(o);
        }
      })
    }
  ...
  public dashStroke = () => {
    ...
    if (activeObject.strokeDashArray && activeObject.strokeDashArray.length !== 0){
      activeObject.setOptions({strokeDashArray: []});
    }
    else {
      activeObject.setOptions({strokeDashArray: [3]});
    }
    this.canvas.renderAll()
  }
  ...
  public changeColor = (color: string) => {
    ...
    if (activeObject && activeObject._objects) {
      ...
      activeObject.fillIndex.forEach((svgIndex: number) => {
        activeObject._objects[svgIndex].set({
          fill: color
        });
      });
    }
  ...
  public getObjectColor = () => {
    const activeObject = this.canvas.getActiveObject() as FabricObject;
    if (activeObject && activeObject._objects.length > 0) {
      return activeObject._objects[activeObject.fillIndex[0]]['fill'];
    } else if (activeObject && activeObject.fill) {
  ...
  private convertURIToImageData = (URI: string) => {
  ...
  public getCanvasImageBlob = (option = { name: 'New Image', format: 'jpeg', quality: 1 }) => {
    this.zoomHandler.zoomOneToOne();
		const { left, top, width, height } = this.workarea;
