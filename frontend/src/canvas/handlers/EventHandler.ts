...
import { tipArrow } from '../objects/Arrow';
...
class EventHandler {
  ...
  constantY: number;
	...
	public moving = (opt: FabricEvent) => {
		const { target, pointer } = opt as any;
    if (target.type == 'pathArrow') {
      this.handler.canvas.remove(this.hideObject);
    } else if (target.parentId) {
      const [aX1, aY1, aX2, aY2] = tipArrow([pointer.x + this.constantX, pointer.y + this.constantY, this.targetObject.path[0][1], this.targetObject.path[0][2]])
      this.targetObject.path[1][1] = pointer.x + this.constantX;
      this.targetObject.path[1][2] = pointer.y + this.constantY;
      this.targetObject.path[3][1] = aX1;
      this.targetObject.path[3][2] = aY1;
      this.targetObject.path[5][1] = aX2;
      this.targetObject.path[5][2] = aY2;
	...
	public moved = (opt: FabricEvent) => {
		const { target } = opt;
    if (target.type == 'pathArrow' && target?.obj_type !== 'sinePath') {
      const offsetX = target.left - this.left;
      const offsetY = target.top - this.top;
      this.hideObject.left = this.hideObject.left + offsetX
      this.hideObject.top = this.hideObject.top + offsetY
      this.left = target.left;
      this.top = target.top;
      this.handler.canvas.add(this.hideObject);
      return;
    }
	...
	public mousedown = (opt: FabricEvent) => {
		...
		if (editable) {
			...
			if (this.handler.interactionMode === 'polygon') {
				...
				if (this.handler.pointArray.length && this.handler.activeLine) {
					this.handler.drawingHandler.arrowpath.generate(event);
				} else {
					this.handler.drawingHandler.arrowpath.addPoint(event);
				}
			} else if (this.handler.interactionMode === 'sineArrow') {
				if (this.handler.pointArray.length && this.handler.activeLine) {
					this.handler.drawingHandler.sinePath.generate(event);
				} else {
					this.handler.drawingHandler.sinePath.addPoint(event);
				}
      }
		}
	};
	...
	public mousemove = (opt: FabricEvent) => {
		...
		if (this.handler.interactionMode === 'polygon') {
			...
			this.handler.canvas.requestRenderAll();
		} else if (this.handler.interactionMode === 'arrow' || this.handler.interactionMode === 'sineArrow') {
			if (this.handler.activeLine && this.handler.activeLine.class === 'line') {
				const pointer = this.handler.canvas.getPointer(event.e);
				this.handler.activeLine.set({ x2: pointer.x, y2: pointer.y });
			}
	...
	public selection = (opt: FabricEvent) => {
		const { onSelect, activeSelectionOption, canvas } = this.handler;

		const target = opt.target as FabricObject<fabric.ActiveSelection>;
    canvas.forEachObject((o: FabricObject, _) => {
      if (target && o?.parentId == target?.id) { 
        this.hideObject = o;
        this.left = target.left;
        this.top = target.top
      } else if (target && target.parentId && o?.id == target.parentId) {
        this.targetObject = o;
        this.constantX = o.path[1][1] - target.left;
        this.constantY = o.path[1][2] - target.top;
      }
    })
		...
		if (this.handler.interactionHandler.isDrawingMode()) {
			if (this.handler.shortcutHandler.isEscape(e)) {
				if (this.handler.interactionMode === 'polygon') {
					...
					this.handler.drawingHandler.arrowpath.finish();
				} else if (this.handler.interactionMode === 'link') {
