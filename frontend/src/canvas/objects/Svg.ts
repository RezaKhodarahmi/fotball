...
const Svg = fabric.util.createClass(fabric.Group, {
	...
	addSvgElements(objects: FabricObject[], options: any, path: string) {
		...
		if (createdObj.getObjects) {
			(createdObj as FabricGroup).getObjects().forEach((obj, index) => {
				this.add(obj);
        if (options.fillIndex && options.fillIndex.includes(index)) {
          obj.set('fill', options.fill);
        } else if (!options.fillIndex && options.fill) {
					obj.set('fill', options.fill);
				}
				if (options.stroke) {
	...
	loadSvg(option: SvgOption) {
		const { svg, loadType, fill, fillIndex } = option;
		return new Promise<SvgObject>(resolve => {
			if (loadType === 'svg') {
				fabric.loadSVGFromString(svg, (objects, options) => {
					resolve(this.addSvgElements(objects, { ...options, fill, fillIndex }, svg));
				});
			} else {
